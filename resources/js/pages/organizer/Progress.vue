...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4">
            <h1 class="text-xl font-semibold text-white">Judging Progress</h1>
            ...
            <div v-else class="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900/80">
                <table class="min-w-full text-left text-sm text-slate-200">
                    ...
                    <tbody>
                        <tr v-for="row in progress" :key="row.judge_id" class="border-b border-slate-800/70">
                            ...
                            <td class="px-4 py-3">
                                <span
                                    ...
                                    :class="{
                                        'bg-slate-600 text-slate-200': row.status === 'not_started',
                                        'bg-amber-500/20 text-amber-300': row.status === 'in_progress',
                                        'bg-[#38F298]/20 text-[#38F298]': row.status === 'submitted',
                                    }"
                                >
                ...
                <p v-if="progress.length === 0" class="p-6 text-center text-slate-400">No judges configured.</p>
            </div>
        </div>
    </AppLayout>
</template>
